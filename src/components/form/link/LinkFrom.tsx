...
export default function LinkFrom({ onFormValidation, setFormData, children }: LinkFormProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const isFormValid = !!watch('url') && !errors.url && selectedPlatform !== null;
  useEffect(() => {
  ...
  return (
    <form className={styles.linkForm} onSubmit={handleSubmit(onSubmitHandler)}>
      ...
      <div className={styles.inputContainer}>
        <Dropdown options={platforms} selectedOption={selectedPlatform} onSelect={handleSelectPlatform} />
        <Input
          label="Link"
          type="text"
          id="url"
